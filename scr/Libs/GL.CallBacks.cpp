...
void keyboard(unsigned char key, int x, int y){
    ...
    if(run)
        mov*=1.5;
        const float maxMov=20;
    if(tolower(key)=='w'){//||tolower(key)=='a'||tolower(key)=='s'||tolower(key)=='d'
    if(Player::livingWell)Player::life+=mov;
    else Player::life-=mov;
    if(ABS(Player::life)>maxMov)Player::livingWell=!Player::livingWell;
    }
}
...
void keyboardUp(unsigned char key,int x,int y){
    switch(key){
        ...
        case 'R':
            case 'r':
                Scenes::cheatMode=!Scenes::cheatMode;
        break;
...
void update(int n){
    ...
    if(glCallbacksReleaseWOffSet)Player::life=0;
    glutPostRedisplay();
...
void mousePassiveMotion(int x,int y){
    ...
    //if(fmod(GL::framesInGame,GL::getFPS()/30)==0){
