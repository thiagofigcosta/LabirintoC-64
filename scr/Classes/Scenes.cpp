...
void Scenes::drawGame(){
    if(!gameCalled){
        ...
        if(Light::useLight)Light::setAmbientLight();      
   }
    if(GL::isPaused){
        GL::drawPause();
    }else{
...
void Scenes::drawOptions(){
  ...
  if(!Light::useFog)GL::drawTexture(Util::nTRectangleSet(100,100,350,200,0.5,0.5),GL::getColorByName("white"),GL::getTextureByName("X"),1);
