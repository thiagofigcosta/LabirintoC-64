...
void Scenes::drawGame(){
    if(!gameCalled){
        ...
        Shader::compileShader("shaders/lightVrtx.vs","shaders/lightFrag.vs",true,true,true);
        Map::scale.setPoint(3,5,0);
        Player::spawn(Util::nTPointSet(Map::scale.x,0,Map::scale.x));
    ...
    if(fmod(GL::framesInGame,GL::getFPS())==0)cout<<GL::cam; //GL::drawObj(GL::loadSimpleObj("obj/juggernault.obj"),GL::getColorByName("red"));
